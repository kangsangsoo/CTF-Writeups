...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let ix = solve::instruction::Initialize {
        name: auction.as_ref().to_vec(),
        seed: rich_boi.as_ref()[0..32].try_into().unwrap(),
    };
    let data = ix.data();
    
    // added
    let user_bid =
        Pubkey::find_program_address(&[auction.as_ref(), user.as_ref()], &chall_id).0;
    let (fake_product, _) = Pubkey::find_program_address(&[auction.as_ref(), rich_boi.as_ref()[0..32].try_into().unwrap()], &chall_id);
